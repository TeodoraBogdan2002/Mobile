...
class _RecordSectionState extends State<RecordSection> {
  ...
  getDates() async {
    ...
    if (online) {
      try {
       ...
       // await DatabaseHelper.updatePetData(dates);
      } catch (e) {
        logger.e(e);
        message(context, "Error connecting to the server", "Error");
      }
    } else {
      dates = await DatabaseHelper.getPets();
    }
  ...
  Future<void> saveMealData(PetData mealData) async {
    if (!mounted) return;
    setState(() {
      isLoading = true;
    });
    if (online) {
      try {
        final PetData received =
            await ApiService.instance.addPetData(mealData);
        DatabaseHelper.addPetData(received);
      ...
      message(context, "Operation available just online", "Error");
    }
    setState(() {
      isLoading = false;
    });
                        ...
                        return ListTile(
                          ...
                          trailing: IconButton(
                            ...
                            onPressed: () {
                              ...
                              context,
                              MaterialPageRoute(
                                builder: (context) => PetDataPage(dates[index].id ?? 0),
                              ),
                            );
                            },
                            color: Colors.red,
