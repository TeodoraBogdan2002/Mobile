...
class _RecordSectionState extends State<RecordSection> {
  ...
  getDates() async {
    ...
    if (online) {
      try {
        dates = await ApiService.instance.getActivities();
        //await DatabaseHelper.updateActivityData(dates);
      } catch (e) {
        logger.e(e);
        message(context, "Error connecting to the server", "Error");
      }
    } else {
      dates = await DatabaseHelper.getActivities();
    }
  ...
  Future<void> saveMealData(ActivityData mealData) async {
    if (!mounted) return;
    setState(() {
      isLoading = true;
    });
    if (online) {
      try {
        final ActivityData received =
            await ApiService.instance.addActivityData(mealData);
        DatabaseHelper.addActivityData(received);
      ...
      DatabaseHelper.addActivityData(mealData);
    }
    setState(() {
      isLoading = false;
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Activity Organizer section'),
      ),
      body: isLoading
                  ...
                  ListView.builder(
                      itemBuilder: ((context, index) {
                        return ListTile(
                          ...
                          subtitle: Text(
                            'TYPE: Name: ${dates[index].name}, Date: ${dates[index].date}, Type: ${dates[index].type}, Status: ${dates[index].status}',
                          ),
                          trailing: IconButton(
